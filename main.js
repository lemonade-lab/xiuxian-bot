...
function App$5({ data }) {
    ...
    return (React.createElement("html", null,
        React.createElement("head", null,
            React.createElement("link", { rel: "stylesheet", href: "../css/root.css" }),
            React.createElement("link", { rel: "stylesheet", href: "../css/information.css" })),
        React.createElement("body", null,
            React.createElement("div", { id: "app", className: "user" },
                React.createElement("div", { style: { display: 'grid' } },
                    React.createElement("div", { className: "user_top" },
                        React.createElement("div", { className: "user_top_left" },
                            React.createElement("div", { className: "user_top_right_font0 font_control Horizontal_grid", style: { padding: '5px' } }, UID),
                ...
                React.createElement("div", { className: "user_bottom1" },
                    React.createElement("div", { className: "use_data" },
                        ...
                        React.createElement("div", { className: "user_top_right_font2 font_control Horizontal_grid" }, data.autograph))),
                React.createElement("div", { className: "user_bottom1" },
                    React.createElement("div", { className: "use_data" },
                        React.createElement("div", { className: "user_top_right_font0 font_control Horizontal_grid" }, '[个人信息]'),
                        React.createElement("div", { className: "user_top_msg" },
                            React.createElement("div", { className: "user_font_msg", style: { color: data.color } },
                                "\u7075\u529B: ",
                        ...
                        React.createElement("div", { className: "user_top_msg" },
                            ...
                            React.createElement("div", { className: "user_font_msg", style: { color: data.color } },
                                "\u5929\u8D4B: ",
                                data.talentsize)),
                        React.createElement("div", { className: "user_top_right_font2 font_control Horizontal_grid grid-4" }, data.equipment.map(item => (React.createElement("div", { key: item['good.name'], style: { color: data.color } }, item['good.name'])))))),
                React.createElement("div", { className: "user_bottom1" },
                    React.createElement("div", { className: "use_data" },
                        React.createElement("div", { className: "user_top_right_font0 font_control Horizontal_grid", style: { color: data.color } }, '[已学功法]'),
                        React.createElement("div", { className: "user_top_right_font2 font_control Horizontal_grid grid-4" }, data.skills.map(item => (React.createElement("div", { key: item['good.name'], style: { color: data.color } },
                            "\u300A",
                            item['good.name'],
                            "\u300B"))))))))));
}
...
mkdirSync(dir$1, { recursive: true });
const p = new Puppeteer();
function getBagComponent(data, name = 'bag.html') {
    const html = renderToString(React.createElement(App$9, { data: data }));
    const address = join(dir$1, name);
    writeFileSync(address, `<!DOCTYPE html>${html}`);
    return p.toFile(address, {});
}
function getDefsetComponent(data, name = 'defset.html') {
    const html = renderToString(React.createElement(App$8, { data: data }));
    const address = join(dir$1, name);
    writeFileSync(address, `<!DOCTYPE html>${html}`);
    return p.toFile(address, {});
}
function getEquipmentComponent(data, name = 'equipment.html') {
    const html = renderToString(React.createElement(App$7, { data: data }));
    const address = join(dir$1, name);
    writeFileSync(address, `<!DOCTYPE html>${html}`);
    return p.toFile(address, {});
}
function getHelpComponent(data, name = 'help.html') {
    const html = renderToString(React.createElement(App$6, { data: data }));
    const address = join(dir$1, name);
    writeFileSync(address, `<!DOCTYPE html>${html}`);
    return p.toFile(address, {});
}
function getInformationComponent(data, name = 'information.html') {
    const html = renderToString(React.createElement(App$5, { data: data }));
    const address = join(dir$1, name);
    writeFileSync(address, `<!DOCTYPE html>${html}`);
    return p.toFile(address, {});
}
function getKillComponent(data, name = 'kill.html') {
    const html = renderToString(React.createElement(App$4, { data: data }));
    const address = join(dir$1, name);
    writeFileSync(address, `<!DOCTYPE html>${html}`);
    return p.toFile(address, {});
}
function getListComponent(data, name = 'list.html') {
    const html = renderToString(React.createElement(App$3, { data: data }));
    const address = join(dir$1, name);
    writeFileSync(address, `<!DOCTYPE html>${html}`);
    return p.toFile(address, {});
}
function getRingComponent(data, name = 'ring.html') {
...
function getSkyComponent(data, name = 'sky.html') {
...
class SkyTower extends APlugin {
    ...
    async showSky(e) {
        ...
        const img = await getSkyComponent(await showSky(UID));
        if (typeof img != 'boolean')
            e.reply(img);
        return;
    }
...
class Bag extends APlugin {
    ...
    async showBagType(e) {
        ...
        const img = await getBagComponent(await backpackInformation(e.user_id, e.user_avatar, mapType[type] ?? mapType['道具']));
        if (typeof img != 'boolean')
            e.reply(img);
        return;
    }
...
class Ring extends APlugin {
    ...
    async showRing(e) {
        ...
        if (!(await isThereAUserPresent(e, UID)))
            return;
        const img = await getRingComponent(await ringInformation(UID, e.user_avatar));
        if (typeof img != 'boolean')
            e.reply(img);
        return;
    }
...
class List extends APlugin {
    ...
    async showList(e) {
        ...
        if (!(await isThereAUserPresent(e, UID)))
            return;
        const img = await getListComponent(await getList());
        if (typeof img != 'boolean')
            e.reply(img);
    }
    async killGodChart(e) {
        ...
        if (!(await isThereAUserPresent(e, UID)))
            return;
        const img = await getKillComponent(await getKillList());
        if (typeof img != 'boolean')
            e.reply(img);
    }
}
