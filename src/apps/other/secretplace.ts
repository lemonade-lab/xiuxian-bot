import { APlugin, Messages, type AEvent } from 'alemonjs'
import {
  isThereAUserPresent,
  sendReply,
  ControlByBlood,
...
message.response(/^(#|\/)?前往[\u4e00-\u9fa5]+$/, async e => {
  const UID = e.user_id
  if (!(await isThereAUserPresent(e, UID))) return
  const UserData = await GameApi.Users.read(UID)
  // 闭关等长期状态自动结束
...
message.response(/^(#|\/)?(传送|傳送)[\u4e00-\u9fa5]+$/, async e => {
  const UID = e.user_id
  if (!(await isThereAUserPresent(e, UID))) return
  const UserData = await GameApi.Users.read(UID)
  if (!(await ControlByBlood(e, UserData))) return
...
export const Secretplace = message.ok
