...
message.response(/^(#|\/)?(击杀|擊殺)[\u4e00-\u9fa5]+$/, async e => {
  const UID = e.user_id
  ...
  const monstersdata = await GameApi.Monster.monsterscache(UserData.point_type)
...
export const Monster = message.ok
